...
class SWMCA(nn.Module):
    def __init__(
        ...
        self.norm_y = nn.LayerNorm(embed_dim)
    ...
    def forward(self, x: Tensor, y: Tensor):
        ...
        """
        B, C, H, W = x.shape
        assert y.shape == (B, C, H, W), "Inputs must share the same shape"
        M = self.window_size
        ...
        if self.shift_size > 0:
            ...
            y = torch.roll(y, self.shift, dims=(1, 2))

        x_windows = create_local_window(x, self.window_size)  # [B,num_windows,M*M,C]
        y_windows = create_local_window(y, self.window_size)  # [B,num_windows,M*M,C]
        _, num_windows, M2, _ = x_windows.shape

        x_flat = x_windows.view(B * num_windows, -1, C)  # [B*num_windows,M*M,C]
        y_flat = y_windows.view(B * num_windows, -1, C)  # [B*num_windows,M*M,C]

        shape_heads = (B * num_windows, M2, self.num_heads, C // self.num_heads)

        q_x = (
            self.q(x_flat).reshape(*shape_heads).permute(0, 2, 1, 3)
        )  # [B*num_windows,NH,M*M,D]
        q_y = (
            self.q(y_flat).reshape(*shape_heads).permute(0, 2, 1, 3)
        )  # [B*num_windows,NH,M*M,D]

        k_x = (
            self.k(x_flat).reshape(*shape_heads).permute(0, 2, 1, 3)
        )  # [B*num_windows,NH,M*M,D]
        k_y = (
            self.k(y_flat).reshape(*shape_heads).permute(0, 2, 1, 3)
        )  # [B*num_windows,NH,M*M,D]

        v_x = (
            self.v(x_flat).reshape(*shape_heads).permute(0, 2, 1, 3)
        )  # [B*num_windows,NH,M*M,D]
        v_y = (
            self.v(y_flat).reshape(*shape_heads).permute(0, 2, 1, 3)
        )  # [B*num_windows,NH,M*M,D]
        ...
        out_x = self.scaled_dot_product_attention(
            q_x, k_y, v_y, b, attn_mask
        )  # [B*num_windows,M*M,C]

        out_y = self.scaled_dot_product_attention(
            q_y, k_x, v_x, b, attn_mask
        )  # [B*num_windows,M*M,C]

        out_x = out_x.reshape(B, num_windows, M2, C)
        out_y = out_y.reshape(B, num_windows, M2, C)

        out_x = reverse_local_window(out_x, self.window_size, H, W)  # [B,H,W,C]
        out_y = reverse_local_window(out_y, self.window_size, H, W)  # [B,H,W,C]

        if self.shift_size > 0:
            out_x = torch.roll(out_x, self.reverse_shift, dims=(1, 2))
            out_y = torch.roll(out_y, self.reverse_shift, dims=(1, 2))

        out_x = self.proj_x(out_x)
        out_y = self.proj_y(out_y)

        out_x = out_x.permute(0, 3, 1, 2)
        out_y = out_y.permute(0, 3, 1, 2)
    ...
    def generate_attn_mask(self, region_id_maps: Tensor) -> Tensor:
        ...
        Returns:
            attn_mask: [B,num_windows,M*M,M*M]
        """
